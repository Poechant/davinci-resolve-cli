...
def test_default_paths_per_platform(platform_name: str, expect_substring: str, lib_suffix: str) -> None:
    ...
    assert api is not None and expect_substring in str(api)
    assert lib is not None and str(lib).endswith(lib_suffix)
...
def test_windows_path_contains_program_files(monkeypatch) -> None:
    ...
    assert api is not None and ("ProgramData" in str(api) or "PROGRAMDATA" in str(api))
    assert lib is not None and "Program Files" in str(lib)
...
def test_windows_program_data_env_override(monkeypatch) -> None:
    ...
    assert api is not None and "D:" in str(api)


def test_linux_paths_are_posix() -> None:
    api, lib = bootstrap.default_paths("Linux")
    assert str(api).startswith("/opt/resolve")
    assert str(lib).startswith("/opt/resolve")
